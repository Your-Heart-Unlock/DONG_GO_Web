...
- [x] 검색 UI(키워드 검색 → 후보 리스트) - **DB 검색 연동 완료 (X섹션)**
...
## X. 메인 지도 키워드 검색 (0.5d) - **완료** → 상세: `IMPL-X_MAP_SEARCH.md`
- [x] 검색 API 생성 (`/api/places/search?keyword=...&limit=10`)
  - [x] `status == 'active'` 장소 대상 이름/주소 검색
  - [x] 이름 시작 일치 우선 정렬
- [x] SearchBar 드롭다운 UI
  - [x] 300ms 디바운스 입력
  - [x] 결과 목록 (이름 + 카테고리 뱃지 + 주소)
  - [x] 로딩/빈 결과 상태
  - [x] 외부 클릭/필터 패널 열림 시 닫기
- [x] 메인 페이지 연동
  - [x] 결과 클릭 → 지도 zoom 16 이동 + 바텀시트 표시
  - [x] 결과 없음 → "식당 추가 검색하기" 버튼 → `/add?q=검색어`
- [x] `/add` 페이지 `?q=` 파라미터 자동 검색
  - [x] `useSearchParams()` + `Suspense` 래퍼
  - [x] 검색 로직 `performSearch()` 분리
